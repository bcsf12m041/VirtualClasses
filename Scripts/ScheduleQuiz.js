$(document).ready(function fun() {
    ...
    $("#idform").on('click', function setTotalQuestions() {
            ...
            var total = parseInt(document.getElementById("idtotalquestions").value);
            var text = "";
            if (total != 0) {
                text = "<hr/>";
            }
            for (i = 1; i <= total; i++) {
                ...
                text += "<label for=\"q" + i + "optionA\">Option A</label>";
                text += "<input type=\"text\" style=\"margin-left: 20px\" class=\"form-control\" name=\"q" + i + "optionA\" placeholder=\"Option A\"></input>";
                text += "</div>";
                text += "<div class=\"form-group\" style=\"margin-left:36%\">";
                text += "<label for=\"q" + i + "optionB\">Option B</label>";
                text += "<input type=\"text\" style=\"position: absolute;right:20px;margin-top:-5px;\" class=\"form-control\" name=\"q" + i + "optionB\" placeholder=\"Option B\"></input>";
                text += "</div></div>";
                text += "<div class=\"form-inline tab\" style=\"margin-top:10px\"><div class=\"form-group\">";
                text += "<label for=\"q" + i + "optionC\">Option C</label>";
                text += "<input type=\"text\" style=\"margin-left: 20px\" class=\"form-control\" name=\"q" + i + "optionC\" placeholder=\"Option C\"></input>";
                text += "</div><div class=\"form-group\" style=\"margin-left:36%\">";
                text += "<label for=\"q" + i + "optionD\">Option D</label>";
                text += "<input type=\"text\" style=\"position: absolute;right:20px;margin-top:-5px;\" class=\"form-control\" name=\"q" + i + "optionD\" placeholder=\"Option D\"></input>";
                text += "</div></div>";
                text += "<hr/>";
            }
            if (total != 0) {
